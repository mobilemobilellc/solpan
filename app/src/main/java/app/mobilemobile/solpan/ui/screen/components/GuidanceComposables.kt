...
fun GuidanceCard(
  ...
  val rollInstruction =
    when {
      isRollCorrect -> stringResource(id = R.string.guidance_roll_level)
      ...
      else -> stringResource(id = R.string.guidance_roll_adjust_level)
    }
  ...
  InfoCard(title = stringResource(id = R.string.guidance_card_title), icon = Icons.Filled.Tune) {
    ...
    if (
      targetParameters.targetMagneticAzimuth == null && targetParameters.magneticDeclination != null
    ) {
      Text(
        text =
          stringResource(
...
fun GuidanceRow(
  ...
  val contentColor =
    if (isCorrect) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurfaceVariant
...
fun GuidanceCardAlignedPreview() {
  SolPanTheme {
    Surface {
      // Panel target magnetic azimuth is 178.0. Phone's top should point to 178.0 for alignment.
