...
class DataStoreUserPreferencesRepository(
    ...
    // An unreadable file would otherwise fail every collector, and uiState collects this.
    override val tutorialSeen: Flow<Boolean> =
        dataStore.data
            .catch { error -> if (error is IOException) emit(emptyPreferences()) else throw error }
            .map { prefs -> prefs[UserPreferencesKeys.TUTORIAL_SEEN] ?: false }

    override suspend fun setTutorialSeen(seen: Boolean) {
        dataStore.edit { prefs -> prefs[UserPreferencesKeys.TUTORIAL_SEEN] = seen }
    }
}
